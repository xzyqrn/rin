...
// ── Helper: Cache prepared statements ──────────────────────────────────────────
function getStmt(db, sql) {
  if (!db._statements) db._statements = {};
  if (!db._statements[sql]) db._statements[sql] = db.prepare(sql);
...
async function saveGoogleTokens(db, userId, tokens) {
  ...
  try {
    ...
    if (existing) {
      ...
      getStmt(sqliteDB, `UPDATE google_auth SET ${sets.join(', ')} WHERE user_id = ?`).run(...values);
    } else {
      getStmt(sqliteDB,
