...
function createHtmlResponse(title: string, message: string, icon: string, isError: boolean = false) {
    const bgColor = isError ? '#fce8e6' : '#e6f8fa';
    const textColor = isError ? '#d32f2f' : '#4caf50';
    return `
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body style="font-family: sans-serif; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0; background: ${bgColor}; flex-direction: column;">
          <h2 style="color: ${textColor};">${icon} ${escapeHtml(title)}</h2>
...
export async function GET(request: Request) {
    ...
    if (error) {
        ...
        return new NextResponse(
            createHtmlResponse('Authentication Failed', `Auth error: ${error}`, '❌', true),
    ...
    if (!code || !state) {
        ...
        return new NextResponse(
            createHtmlResponse('Missing Parameters', 'Missing code or state', '❌', true),
            { status: 400, headers: { 'Content-Type': 'text/html' } }
        );
    }
    ...
    try {
        ...
        if (!db) {
            ...
            return new NextResponse(
                createHtmlResponse('Database Error', 'Database not configured', '❌', true),
        ...
        return new NextResponse(
            createHtmlResponse('Google Account successfully linked!', 'Your tokens have been saved to the database.', '✅'),
        ...
        return new NextResponse(
            createHtmlResponse('Internal Error', 'Internal Server Error during authorization.', '❌', true),
            { status: 500, headers: { 'Content-Type': 'text/html' } }
        );
    }
}
