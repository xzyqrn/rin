...
import { errorHtmlResponse, SUCCESS_HTML } from '@/lib/html-response';
...
export async function GET(request: Request) {
    ...
    if (error) {
        ...
        return errorHtmlResponse(`Auth error: ${error}`, 400);
    }
    if (!code || !state) {
        console.error('[Google Callback] Missing parameters:', { hasCode: !!code, hasState: !!state });
        return errorHtmlResponse('Missing code or state', 400);
    }
    ...
    try {
        ...
        if (!db) {
            ...
            return errorHtmlResponse('Database not configured', 500);
        }
        ...
        return errorHtmlResponse('Internal Server Error during authorization.', 500);
    }
}
