...
function getErrorHtml(message: string) {
    ...
    return `
      <html>
        ...
        <body style="font-family: sans-serif; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0; background: #ffebee; flex-direction: column; text-align: center; padding: 0 20px; box-sizing: border-box;">
          <h2 style="color: #d32f2f; font-size: 24px;">❌ Authentication Failed</h2>
          <p style="color: #333; font-size: 16px; margin: 10px 0 20px;">${escapedMessage}</p>
          <p style="color: #666; font-size: 14px;">You can safely close this window.</p>
...
export async function GET(request: Request) {
    ...
    if (error) {
        ...
        return new NextResponse(getErrorHtml(`Auth error: ${error}`), { status: 400, headers: { 'Content-Type': 'text/html' } });
    }
    if (!code || !state) {
        console.error('[Google Callback] Missing parameters:', { hasCode: !!code, hasState: !!state });
        return new NextResponse(getErrorHtml('Missing code or state'), { status: 400, headers: { 'Content-Type': 'text/html' } });
    }
    ...
    if (!stateCheck.ok) {
        ...
        return new NextResponse(
            getErrorHtml(isConfigError ? 'OAuth state verification is not configured on the server.' : 'Invalid or expired OAuth state. Please run /linkgoogle again.'),
    ...
    try {
        ...
        if (!db) {
            ...
            return new NextResponse(getErrorHtml('Database not configured'), { status: 500, headers: { 'Content-Type': 'text/html' } });
        }
        ...
        return new NextResponse(getErrorHtml('Internal Server Error during authorization.'), { status: 500, headers: { 'Content-Type': 'text/html' } });
    }
}
